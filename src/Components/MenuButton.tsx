...
type Props = {
	 ...
	 * button's kind
	 */
	kind?: "dropdown-button" | "dropdown-item" | null;
};
...
export const MenuButton = ({
	...
	if (tooltipData != null && typeof tooltipData !== "string") {
		...
		if (tooltipData.placement) {
			...
			placement = kind === "dropdown-item" ? "left" : "bottom";
		}
	}
	...
	return (
		<button
			...
			className={`s-editor-btn s-btn js-editor-btn js-${id} d-flex ai-center ${
				active ? "is-selected" : ""
			} ${disabled ? "is-disabled" : ""} ${cssClasses.join(" ")}`}
			style={cssStyles}
			...
			data-controller={controller} // Tooltip!
			data-action={isDropdown ? "s-popover#toggle" : null}
			data-key={id}
		>
			{iconName ? (
				<span
